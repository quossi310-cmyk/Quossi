...
function gatingAllow(userId: string, history: ChatMsg[] | undefined): { allowed: boolean; reason?: string } {
  const uTurns = countUserTurns(history || []);
  if (uTurns < MIN_USER_TURNS) {
    return { allowed: false, reason: `Need at least ${MIN_USER_TURNS} user messages before Q-Score.` };
  }
  ...
  if (uTurns - last < REFRESH_EVERY_USER_TURNS) {
    ...
    return { allowed: false, reason: `Learning… Q-Score updates after ${remaining} more user messages.` };
  }
  return { allowed: true };
}
...
function selectWindowByUserTurns(history: ChatMsg[] = []): ChatMsg[] {
  ...
  const targetTurns = Math.min(Math.max(WINDOW_USER_TURNS, MIN_USER_TURNS), MAX_WINDOW_USER_TURNS);
...
function buildAnalysisText(history: ChatMsg[] | undefined, message?: string): { text: string; usedHistory: boolean } {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { text: analysisText, usedHistory } = buildAnalysisText(history, singleMessage);
    if (!analysisText) {
      return Response.json({ ok: false, error: "message or history is required" }, { status: 400 });
    }
    ...
    const firstTask = Array.isArray(tasks) && tasks.length ? String(tasks[0]) : "";
    ...
    return Response.json(
      {
        ...
        userId,
        usedHistory,
        metrics: { clarityPulse, confidenceIndex, qScore, range: story?.range },
        story: story?.message ?? "",
        tasks,
      },
      { status: 200 }
