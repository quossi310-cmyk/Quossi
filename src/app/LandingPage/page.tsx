...
// 🔧 Use only real env vars (no localhost fallbacks)
...
export default function Home() {
  ...
  // 🆕 central image upload + analysis handler
  async function handleFiles(files: FileList | null) {
    ...
    try {
      ...
      const finalized = withPreview.map((m) =>
        m.id === now
          ? { ...m, imagePreview: undefined, imageUrl: publicUrl, status: "sent" }
          : m
      );
      setMessages(finalized);
      localStorage.setItem(STORAGE_KEY, JSON.stringify(finalized));

      // 4) 🧠 ask Quossi XAI to analyze the image
      await analyzeImageWithQuossi(publicUrl, finalized);
    } catch (e: any) {
      console.error(e);
